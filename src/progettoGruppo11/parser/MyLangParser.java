...
public class MyLangParser implements Parser {
	...
	/*
	 * parses a statement Stmt ::= 'var'? IDENT '=' Exp | 'print' Exp | 'if' '(' Exp ')' Block ('else' Block)? | 'foreach' IDENT 'in' Exp Block
	 */
	...
	private ForStmt parseForStmt() throws ParserException{
		...
		final var var = parseVariable();
		consume(IN);
		return new ForStmt(var, parseExp(), parseBlock());
	}
	 ...
	 */
	private Vect parseVect() throws ParserException{
		consume(OPEN_VECT);
		final var exp1 = parseExp();
		consume(SEPARATOR);
		final var exp2 = parseExp();
		consume(CLOSE_VECT);
		return new Vect(exp1, exp2);
	}
